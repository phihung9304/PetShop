        ...
        <form action="{{ route('care-guides.update', $guide->id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select name="species" class="form-control" required>
                    ...
                    <option value="Chó" {{ $guide->species == 'Chó' ? 'selected' : '' }}>Chó</option>
                    <option value="Mèo" {{ $guide->species == 'Mèo' ? 'selected' : '' }}>Mèo</option>
                    <option value="Hamster" {{ $guide->species == 'Hamster' ? 'selected' : '' }}>Hamster</option>
                    <option value="Thỏ" {{ $guide->species == 'Thỏ' ? 'selected' : '' }}>Thỏ</option>
                </select>
            </div>

            <div class="mb-3">
                <label class="fw-bold">Giống</label>
                <input type="text" name="breed" class="form-control" value="{{ $guide->breed }}"
                    placeholder="Nhập giống (ví dụ: Poodle, Alaska)" required>
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea name="content" class="form-control" rows="5" placeholder="Nhập nội dung" required>{{ $guide->content }}</textarea>
            </div>
